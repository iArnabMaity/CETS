...
export default function EmployerAuthPage({ user, setUser, onBack }) {
  const [isLogin, setIsLogin] = useState(true);
  const role = 'employer'; 
  const [formData, setFormData] = useState({ name: '', email: '', password: '', confirm_password: '', company_name: '', phone: '', incorporation_year: '' });
    ...
    try {
        if (isLogin || isAdmin) {
            ...
            if (response.data.role !== 'employer') {
                setError("Access Denied: You are not authorized for the Corporate portal.");
                setLoading(false);
                return;
            }
            ...
            setTimeout(() => {
                setSuccessMsg(''); setIsLogin(true); setCaptchaStatus('idle'); setFormData({ name: '', email: '', password: '', confirm_password: '', company_name: '', phone: '', incorporation_year: '' }); setUniqueErrors({ email: '', phone: '' });
                const alignments = ['justify-start', 'justify-center', 'justify-end'];
                setCaptchaAlign(alignments[Math.floor(Math.random() * alignments.length)]);
            }, 3000);
        }
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4">
              ...
              <>
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" name="company_name" placeholder="Company / Organization Name" required
                  ...
                  <div className="relative group">
                    ...
                    <input type="number" name="incorporation_year" placeholder="Year of Incorporation" required
                      min="1800" max="2026"
                      value={formData.incorporation_year} onChange={handleInputChange}
                      className={`w-full pl-12 pr-4 py-4 text-sm rounded-2xl border outline-none focus:ring-2 focus:ring-${t.accent}-500/50 transition-all duration-300 backdrop-blur-sm bg-[#0f172a] border-slate-700/50 text-white placeholder-slate-500 hover:bg-slate-900/60`}
                      style={{ backgroundColor: '#0f172a', color: '#f1f5f9' }}
                    />
                  </div>
